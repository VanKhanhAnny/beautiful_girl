  ...
  return (
    <div className={className}>
      <div className="flex flex-col items-center w-full">
        {/* AnimationManager moved down, now in a column layout */}
        <AnimationManager
        ...
        <div className="mt-4 w-full">
          {!microphone ? (
            <div className="text-base text-black text-center w-full">Loading microphone...</div>
          ) : (
            <Fragment>
              {socketState === -1 && requiresUserActionToInitialize && (
                <button className="text-center w-full" onClick={handleVoiceBotAction}>
                  <span className="text-xl text-black">Tap to start!</span>
                </button>
              )}
              {socketState === 0 && (
                <div className="text-base text-black text-center w-full font-medium">Loading SafeGuard...</div>
              )}
              {socketState > 0 && status === VoiceBotStatus.SLEEPING && (
                <div className="text-xl flex flex-col items-center justify-center mt-4 mb-4">
                  <div className="text-black text-sm">
                    I&apos;ve stopped listening. {isMobile ? "Tap" : "Click"} the image to resume.
                  </div>
                </div>
              )}
              {/* Transcript Section */}
              <div
                className={`h-20 md:h-12 text-sm md:text-base mt-2 flex flex-col items-center text-black overflow-y-auto`}
              >
